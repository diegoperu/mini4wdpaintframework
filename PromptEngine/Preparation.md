...
A schematic diagram showing the body shell disassembled into its main components (body, chassis, wheels, windows). Each component labeled with a small arrow and Source Sans Pro Regular 8pt DarkGray label. Lines from labels use MidGray. Background: white. Note: if render not available, describe as [EXPLODED VIEW PLACEHOLDER].
...
- Separator: 1px LightGray horizontal rule, full column width, below tip/warning, 8px margin above next step.

Preparation steps from PROJECT.yaml:
{{preparationSteps}}

**TIPS BOX C009 (below step list, before footer, full width minus margins):**
Gold-left-border box: border-left 4px GoldAccent, background #FFFDF0, border-radius 0 3px 3px 0, padding 10px.
Icon: lightbulb [*] in GoldAccent, 14pt.
Heading: "PRO TIPS" Source Sans Pro Bold 9pt GoldAccent.
...
Save output to: `Projects/{Model}/{Variant}/ApprovedText/P004/content.yaml`
